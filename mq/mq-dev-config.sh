...
configure_tls()
{
  ...
  if [ "${MQ_DEV}" == "true" ]; then
    su -l mqm -c "echo \"ALTER CHANNEL('SAIF.APP.SVRCONN') CHLTYPE(SVRCONN) SSLCIPH(TLS_RSA_WITH_AES_256_GCM_SHA384) SSLCAUTH(OPTIONAL)\" | runmqsc $1"
    su -l mqm -c "echo \"ALTER CHANNEL('SAIF.ADMIN.SVRCONN') CHLTYPE(SVRCONN) SSLCIPH(TLS_RSA_WITH_AES_256_GCM_SHA384) SSLCAUTH(OPTIONAL)\" | runmqsc $1"
  fi
}
...
MQ_DEV=${MQ_DEV:-"true"}
MQ_ADMIN_NAME="admin"
MQ_ADMIN_PASSWORD=${MQ_ADMIN_PASSWORD:-"password"}
MQ_APP_NAME="app"
...
su -l mqm -c "setmqaut -m $1 -n \"SAIF.**\" -t queue -g mqclient +put +get +browse"
su -l mqm -c "setmqaut -m $1 -n \"SAIF.**\" -t topic -g mqclient +sub +pub"
...
if [ "${MQ_DEV}" == "true" ]; then
  ...
  echo "ALTER CHANNEL('SAIF.APP.SVRCONN') CHLTYPE(SVRCONN) MCAUSER('${MQ_APP_NAME}')" | runmqsc $1

  # If client password set to "" allow users to connect to application channel without a userid
  if [ "${MQ_APP_PASSWORD}" == "" ]; then
    echo "SET CHLAUTH('SAIF.APP.SVRCONN') TYPE(ADDRESSMAP) ADDRESS('*') USERSRC(CHANNEL) CHCKCLNT(ASQMGR) ACTION(REPLACE)" | runmqsc $1
  fi
  set -e
fi
